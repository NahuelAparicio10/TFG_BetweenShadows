...
public class ComboController
{
    ...
    {
        // avoid hard reset on brief toggles; only reset when no attack is currently running
    ...
    public void AddInputToSequence(Enums.AttackInputs action, Enums.CharacterState state, Action<string, float> playAnimation)
    {
        ...
        // if combo is active and buffer already covers all steps, ignore extra inputs until finish
        ...
        {
            // If input doesn't match next step, try to ramify (search for any combo that can be matching the sequence)
            if (!_activeCombo.IsNextInput(action, _currentSequence.Count))
            {
                // do not attempt fallback if we are already past the last step
    ...
    private void ExecuteAttackAtCurrentStep(Enums.CharacterState state, Action<string, float> playAnimation)
    {
        ...
        if (!_currentAttack.Execute(state, playAnimation))
        {
